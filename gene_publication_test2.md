...
({symbol2rs}) => {
  ...
  return symbol2rs.results.bindings.map(x => x.dbsnp.value.replace(prefix, "dbsnp:")).join(" ");
...
//  symbol2rs.results.bindings.map(x => array.push('"' + x.dbsnp.value.replace(prefix, "") + '"'));
  symbol2rs.results.bindings.map(x => array.push( x.dbsnp.value.replace(prefix, "")));
  return array;
}
```
...
({rs2pmid_litvar, shaping_pmidinfo}) => {
  if(Object.keys(shaping_pmidinfo).length){
    let pmids = rs2pmid_litvar.filter(i => Object.keys(shaping_pmidinfo).indexOf(i)== -1).map(x => x.replace(/^/, "pubmed:")).join(" ")
    if(!pmids){
      return "' '"
    }else{
      return pmids
